...
if(Modernizr.webgl) {
	...
	d3.queue()
		.defer(d3.csv, "data/ttw_map.csv")
		.defer(d3.csv, "data/ttw_bars.csv")
		.defer(d3.json, "data/config.json")
		.defer(d3.json, "data/geogtopottw.json")

		.await(ready);


function ready (error, data1, data2, config, geog){
		console.log("map, bars");
		map_data = data1;
		bars_data = data2;
		avgLines = [];
		avgLines = bars_data.shift();
		console.log(map_data);
		console.log(bars_data);
		delete avgLines.AREANM;
		delete avgLines.AREACD;
		console.log(avgLines);
...
//console.log(dvc.map.varcolour,dvc.map.numberBreaks);
 ...
 //y([d.key])+25;
			 map_data.forEach(function(d) {
				 //console.log(d);
				 	//console.log(d["High Technology Manufacturing"]);
				  countById[d.AREACD] = +eval(d[dvc.map.mapVarName]); // mapvar
		...
		areas.features.map(function(d,i) {
		  if(!isNaN(countById[d.properties.AREACD]))
		  	{d.properties.fill = color(countById[d.properties.AREACD]);
					//console.log(color(countById[d.properties.AREACD]));
				}
		...
		var values =  map_data.map(function(d) { return +eval(d[dvc.map.mapVarName]); }).filter(function(d) {return !isNaN(d)}).sort(d3.ascending);
...
console.log(countById)
		...
		areas.features.map(function(d,i) {
			console.log()
				console.log(colorScale(countById[d.properties.AREACD]));
		  d.properties.fill = colorScale(countById[d.properties.AREACD]);
			//console.log(countById[d.properties.AREACD] );
        ...
        // }
				...
				// 	//console.log(e,e.features[0].properties.AREACD)
	...
	function selectlist(datacsv) {
			...
			// Build option menu for industry
			var optns = d3.select("#industryDD").append("div")
				.attr("id","sel").append("select")
				.attr("id","areaselect")
				.attr("style","width:98%")
	...
	//	varnames.shift();
						var codenames = [];
						varnames.forEach(function(d,i){
							codenames.push(i);
						});
					var indmenu = d3.zip(varnames,codenames);

					var indust = d3.select("#selectNav")
										.append("div")
										.attr("id","selind")
										.append("select")
					...
					indust.selectAll("p").data(indmenu)
							.enter().append("option")
							.attr("value", function(d){ return d[1]})
							.text(function(d){ return d[0]});
					...
					//	myId=null;
						var ic = 0;
						$('#industselect').chosen({width: "98%", allow_single_deselect:true})
						.on('change', function(evt,params){
console.log(evt,params.selected);
								if(typeof params != 'undefined') {
								ic = varnames[parseInt(params.selected)];
...
function drawGraphic(industryCode){
...
console.log(varnames)
				...
				// })
				//
				// map_data.forEach( function(d) { // for each row of graphic data
		...
		//barg = g.append('g').attr('class','areasbars' );
			...
			//	barg = g.append('g').selectAll('group')
				...
				// contextLines = map_data.filter(function(d) {return d.AREACD == dvc.bar.lineVar})[0];
				// console.log(map_data, contextLines);
				//
				console.log(y.bandwidth())
				console.log(avgLines);

				g.append('g').attr("class", "lines").selectAll("line")
					.data(d3.entries(avgLines)) //d3.entries(contextLines))
					.enter()
					.append("line")
					.attr("class", "line")
					.attr("x1", function(d) { return x(d.value) })
					.attr("x2", function(d) { return x(d.value) })
					.attr("y1", function(d) { return y([d.key]) + 25; })
					.attr("y2", function(d) { return y([d.key])  + 55; })
...
function updateBars(code) {
		...
		var defaultData = bars_data.filter(function(d) {
		//	console.log(d, code);
			return d.AREACD == code;  });

	//	console.log(map_data[2]);
	delete defaultData[0].AREANM;
	delete defaultData[0].AREACD;
...
//barg = g.append('g').attr('class','areasbars' );
	//barg.append('g').attr("class", "bars");
			barg.selectAll("rect")
					.data(d3.entries(defaultData[0]))
					.enter()
					.append('rect')
					.attr("class", "bar")
					.attr("height", 30) //y.bandwidth())
			...
			// get all the rows in map_data concerning current AREACD.
		//	var data = bars_data.filter(function(d) {return d.AREACD == code})
			...
			// if (prevMax == newMax) {
			// 	// no change in domain so just move the bars
			// 	barg.selectAll('rect')
